...
const ViewExamPage: React.FC<{ examData: ExamData }> = ({ examData }) => {
    ...
    function startExam(setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .post(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/start`)
    ...
    function endExam(setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .patch(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/finish`)
            .failure(err => messageApi.error(err))
            .error(err => messageApi.error(err))
            .success(() => {
                setQents(null);
    ...
    function attemptQues(quesID: string, provided: number[], incTrialCount: () => void, setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .patch(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/attempt/${quesID}`)
            .body({ provided })
            .failure(err => messageApi.error(err))
            .error(err => messageApi.error(err))
