...
const EditExamPage: React.FC<{ examData: ExamData }> = ({ examData }) => {
    ...
    function updateExamData(newData: EDataDynamic, setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .patch(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}`)
            .body(newData)
    ...
    function deleteExam(setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .delete(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}`)
            .failure(err => messageApi.error(err))
            .error(err => messageApi.error(err))
    ...
    function saveQues(index: number, quesID: string | null, quesData: QDataDynamic, setSpinner?: (loading: boolean) => void) {
        ...
        if(isNew) {
            exf
            .post(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/ques`)
            .success(res => {
            ...
            exf
            .patch(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/ques/${quesID}`)
            .success(() => {
    ...
    function deleteQues(index: number, quesID: string, setSpinner?: (loading: boolean) => void) {
        ...
        new ExcoFetcher()
            .needAuth()
            .delete(`${process.env.REACT_APP_SERVER_URL}/api/exam/${examData.examID}/ques/${quesID}`)
            .failure(err => messageApi.error(err))
            .error(err => messageApi.error(err))
            .success(() => {
